...
contract NFTLockProxy is IERC721Receiver, Ownable {
    ...
    // */
    // function lock(address fromAssetHash, uint64 toChainId, bytes memory toAddress, uint256 tokenId) public returns (bool) {
    //     (bool ok, string uri) = _transferToContract(fromAssetHash, tokenId);
    //     require(ok, "transfer NFT from fromAddress to lock_proxy contract failed!");
    ...
    //     emit LockEvent(fromAssetHash, _msgSender(), toChainId, toAssetHash, toAddress, tokenId);
    ...
    // function _transferToContract(address fromAssetHash, uint256 tokenId) internal returns (bool, string) {
    //     require(msg.value == 0, "there should be no ether transfer!");
        
    //     IERC721Metadata nft = IERC721Metadata(fromAssetHash);
    //     nft.safeTransferFrom(_msgSender(), address(this), tokenId);
    //     string uri = nft.tokenURI(tokenId);
    //     return (true, uri);
    // }
